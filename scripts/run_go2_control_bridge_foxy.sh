...
if [[ "${ROBOT_SCOPE_CONTROL_ENABLED:-0}" != "1" ]]; then
  echo "[Robot Scope] control bridge is disabled" >&2
  exit 2
fi
if [[ "${ROBOT_SCOPE_CONTROL_TRANSPORT:-}" != "udp" ]]; then
  echo "[Robot Scope] robot-side bridge requires the fixed udp transport" >&2
  exit 2
fi
if [[ "${#ROBOT_SCOPE_CONTROL_BRIDGE_KEY}" -lt 32 ]]; then
  echo "[Robot Scope] control bridge key must contain at least 32 characters" >&2
  exit 2
fi
