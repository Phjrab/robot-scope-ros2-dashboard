...
class WirelessMappingDocumentationContractTests(unittest.TestCase):
    ...
    def test_acceptance_keeps_deployment_motion_and_evidence_explicit(self):
        for contract in (
            ...
            "| HW-6 compound load | `PASS` for 60 s and 10 min; 60 min `NOT_RUN` |",
            "current hardware status remains `MAPPING_STATIONARY_PASS`",
            "`websockets 15.0.1`",
            "`10.021 Hz`",
            "exactly 6",
    ...
    def test_deployment_plan_preserves_network_control_and_private_state(self):
        for contract in (
            ...
            "HW-5 is `MAPPING_STATIONARY_PASS`; HW-6 short gates are",
            "60-minute gate remains `NOT_RUN`",
            "No Nav2 or `SOAK_PASS`",
            "about 502 Hz",
            "about 10 Hz",
