...
$('#refreshButton').addEventListener('click', async () => { await Promise.all([refreshState(), refreshTopics(), refreshSources(), refreshCameraCatalog(), refreshDatasetCapture(), refreshDatasetSessions({ preferredId: selectedDatasetSessionId, forceDetail: true }), refreshMappingControl(), refreshControlSnapshot(), refreshControlBridgeService(true), refreshNavigation(), refreshNavigationParameters(true), refreshServiceLifecycle(true)]); showToast('대시보드를 갱신했습니다.'); });
