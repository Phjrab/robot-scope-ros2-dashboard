# Robot Scope — 상세 설치·운영 안내

이 문서는 기존 README의 상세 설치, 설정, 화면별 조작, 카메라, 서비스, API,
테스트 및 보안 설명을 보존합니다. 현재 구현·검증 상태와 포트폴리오 요약은
[루트 README](../README.md)를 먼저 확인하세요. 아래의 현장 명령은 장비·네트워크와
운영자 안전 조건을 확인한 뒤 해당 [설치 가이드](INSTALL.md)와 함께 사용합니다.

## 설치 및 운영 문서

| 문서 | 내용 |
|---|---|
| [2026-09-01 당시 진행 기록](CURRENT_PROGRESS_AND_NEXT_STEPS_2026-09-01.md) | 당시 완료·보류·차단 항목; 최신 상태는 위 요약과 후속 문서 참조 |
| [현재 아키텍처](ARCHITECTURE.md) | subsystem 소유권, safety boundary, Phase 0 대비 구조와 남은 부채 |
| [대회용 분산 아키텍처](COMPETITION_SYSTEM_ARCHITECTURE.md) | 내부 Jetson·외부 Orin·노트북 책임, 무선 데이터 경계와 WP 기준선 |
| [설치](INSTALL.md) | `observer`, `go2`, `go2-control`, `go2-xt16`, `go2-nav` 모드와 스모크 테스트 |
| [의존성](DEPENDENCIES.md) | 외부 ROS workspace, pin/라이선스 기록과 미포함 구성 요소 |
| [토폴로지](TOPOLOGY.md) | 단일/두 호스트 배선, 서비스 역할과 관리망 분리 |
| [문제 해결](TROUBLESHOOTING.md) | DDS, XT16, 저장, 카메라, 제어와 Nav 진단 순서 |
| [Cockpit 운영자 가이드](COCKPIT_OPERATOR_GUIDE.md) | Layout, Xbox, STOP, Takeover, Mission과 대회 전후 절차 |
| [Cockpit 통합 Acceptance](COCKPIT_ACCEPTANCE.md) | software evidence, 성능·soak·hardware 결과와 P0 |
| [AI 데이터셋](AI_DATASET.md) | 듀얼 카메라 서버 수집, 저장 구조, 라벨링과 YOLO/UFLD 배포 판단 |
| [업데이트/롤백](UPDATE_ROLLBACK.md) | 지도·상태 보존, fast-forward update와 안전 롤백 |
| [대회 release runbook](WP08_RELEASE_LOCK_ROLLBACK_RUNBOOK.md) | offline bundle, Competition Lock, 시작·종료와 commit/model rollback |
| [하드웨어 인수 검증](HARDWARE_ACCEPTANCE.md) | 읽기 전용 Jetson/Go2/XT16 점검, 감독 시나리오와 fail-stop 보고서 |
| [Track C2 controller odometry ADR](ADR_COMPETITION_FASTLIO_CONTROLLER_ODOM.md) | wireless FAST-LIO 기반 명시적 controller odometry와 NG0/NG1 안전 경계 |
| [Track C2 no-goal acceptance](TRACK_C2_COMPETITION_FASTLIO_NO_GOAL_ACCEPTANCE.md) | stationary 센서·controller odometry·NG0 실행 및 cleanup 판정 |
| [3D/2D map-family lineage ADR](ADR_3D_2D_MAP_FAMILY_LINEAGE.md) | PCD와 파생 Nav2 지도 사이의 private exact-revision 계보 계약 |
| [Track D0 map lineage acceptance](TRACK_D0_MAP_LINEAGE_ACCEPTANCE.md) | hardware-free 계보 저장·편집·snapshot 검증 결과와 D1 gate |
| [Offline 3D relocalization engine ADR](ADR_3D_RELOCALIZATION_REGISTRATION_ENGINE.md) | bounded C++17 3DoF registration과 strict process boundary |
| [Track D1 offline registration acceptance](TRACK_D1_OFFLINE_REGISTRATION_ACCEPTANCE.md) | synthetic corpus 정확도·성능과 live-use gate |
| [Stationary 3D relocalization ADR](ADR_STATIONARY_3D_RELOCALIZATION_JOB.md) | exact-family candidate job, fixed source와 no-apply safety boundary |
| [Track D2 live candidate acceptance](TRACK_D2_LIVE_CANDIDATE_ACCEPTANCE.md) | hardware-free 결과와 별도 live deployment gate |
...
자세한 pinned revision은 [의존성 manifest](DEPENDENCIES.md)를 확인하세요.
...
새 호스트에는 [설치 가이드](INSTALL.md)의 mode별 절차를 권장합니다. 설치 helper와
...
[하드웨어 인수 검증 문서](HARDWARE_ACCEPTANCE.md)를 확인하세요.
...
일반/legacy bridge fallback의 전후 상한은 0.30 m/s입니다. 명시적 Go2 프로필
`config/go2.json`은 `max_linear_x=1.0`, `max_linear_y=0.2`,
`default_speed_scale=0.35`, `navigation_speed_scale=1.0`을 설정합니다.
설정 상한은 실측 자율주행 속도나 정지 성능이 아닙니다. 화면 속도 슬라이더는
해당 프로필의 상한 안에서 비율을 조절합니다. 수동 브라우저 입력은 200 ms가 지나면
...
Nav2 입력 freshness `navigation_command_timeout_s=0.30`과 C4 odometry
callback arrival gap 판정 `0.25 s`는 다른 기준입니다. robot-side bridge의
수신 watchdog `0.20 s`도 별도입니다. 현재 Nav2 `0.30 s` 계약은
[합성 timing replay](NAV2_OFFLINE_TIMING_REPLAY_20260912.md)로 확인했습니다.
...
[`docs/lab-network.md`](lab-network.md)에 날짜와 확인 상태별로 기록합니다.
...
Jetson AI 사양과 권장 학습/추론 분리는 [AI 데이터셋 가이드](AI_DATASET.md)를 봅니다.
...
결과는 [Cockpit 통합 Acceptance](COCKPIT_ACCEPTANCE.md)에 software test와 분리해
기록합니다.
...
현재 소유권과 Phase 0 대비 변경은 [아키텍처 문서](ARCHITECTURE.md)를 기준으로
...
- Settings의 진단 ZIP은 지원 공유용 공개 projection이지만 사용자 인증 자료가 아닙니다.
  ...
  [Phase 13 계약](ARCHITECTURE_PHASE13.md)을 따릅니다.
