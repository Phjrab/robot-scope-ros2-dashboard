...
`robot-scope.perception-result/v1` and carries source/boot identity, a
sidecar-monotonic result sequence, exact relay `source_sequence` and
`source_epoch`, robot-monotonic capture and inference timestamps, exact model
ID/hash/backend, input dimensions, status, confidence and a task payload. The
external policy is the sole allowlist for source ID, model identity,
dimensions and object class names. A remote payload cannot add or rename a
class.
...
stale robot-relative input or completion timestamps; invalid boot/sequence
changes; malformed or
...
relay source sequence/epoch, input age, receive age, inferred FPS/latency,
`LIVE`/`DEGRADED`/`STALE`/`OFFLINE` state and an explicit `SHADOW`
...
4. From the external Orin, verify health/latest/history, SHADOW overlays, exact
   source IP, model hashes, result/source sequence advance, source epoch, input
   age and gray stale cleanup.
