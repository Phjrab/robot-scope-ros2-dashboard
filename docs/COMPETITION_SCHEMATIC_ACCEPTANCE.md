...
> 아래 첫 상태표의 `deployed: NOT DONE`은 최초 로컬 구현 시점의 판정입니다.
> 이 문서 하단에는 이후 공개·배포 승인 기록이 이어집니다. 승인 자체를
> 실제 배포 성공으로 읽지 말고, 배포 여부와 release ID는 별도 운영 기록으로
> 확인해야 합니다. 실제 로봇 주행·경기 성공 판정은 여전히 별도입니다.
